...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Archive');
}
...
const tools: McpToolExport['tools'] = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object' as const,
      properties: {
        id: { type: 'string', description: 'Archive.org item identifier (e.g., "nasa").' },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'list_files':
      return listFiles(args.id as string, (args.limit as number) ?? 30);
    default:
...
async function search(query: string, limit: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/advancedsearch.php?${params}`);
  if (!res.ok) throw await httpError(res, 'Internet Archive error');
...
async function getMetadata(id: string) {
  const res = await pwFetch(`${BASE_URL}/metadata/${encodeURIComponent(id)}`);
  if (!res.ok) throw await httpError(res, 'Internet Archive error');
...
async function listFiles(id: string, limit: number) {
  if (!id || !id.trim()) throw new Error('Required argument "id" is missing. Pass an archive.org item identifier like "nasa".');
  const n = Math.max(1, Math.min(1000, Math.floor(limit) || 30));
  const res = await pwFetch(`${BASE_URL}/metadata/${encodeURIComponent(id)}`);
  if (!res.ok) throw await httpError(res, 'Internet Archive error');
  const data = (await res.json()) as { files?: { name?: string; format?: string; size?: string }[]; files_count?: number };
  const all = Array.isArray(data.files) ? data.files : [];
  return {
    id,
    files_count: typeof data.files_count === 'number' ? data.files_count : all.length,
    files: all.slice(0, n).map((f) => {
      const clean = (typeof f.name === 'string' ? f.name : '').replace(/^\/+/, '');
      return {
        name: f.name ?? null,
        format: f.format ?? null,
        size_bytes: f.size != null ? parseInt(f.size, 10) : null,
        download_url: `https://archive.org/download/${encodeURIComponent(id)}/${encodeURIComponent(clean)}`,
      };
    }),
  };
}
...
async function waybackCheck(url: string) {
  ...
  const res = await pwFetch(`${BASE_URL}/wayback/available?${params}`);
  if (!res.ok) throw await httpError(res, 'Wayback Machine error');
