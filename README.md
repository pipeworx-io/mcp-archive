...
`tools/list` at `https://gateway.pipeworx.io/archive/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/archive_search \
  -H 'Content-Type: application/json' \
  -d '{"query":"subject:astronomy"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/archive_search`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
